 ...
 * 
 */
package com.itpro.airtime.charging.db;
...
import com.itpro.airtime.charging.structs.TerminatedEvent;
   ...
   public int getChargingTransactionId() throws SQLException{
    ...
    public   Vector<OfferRecord>  getListOfferRecord() throws SQLException {
        ...
        while(rs.next()) {
            ...
            offerRecord.old_paid_value = rs.getInt("old_paid_value");
			offerRecord.remark = rs.getString("remark");
	...
	public void insertCDRRecord(CDRRecord cdrRecord) throws SQLException {
		...
		String sql = "INSERT INTO cdr(date_time,msisdn,offer_id,offer_type,package_name,package_value,"
				+ "service_fee,charge_value,result_code,result_string,status,transactionID,spID,serviceID,paid_value_before,province_code,sub_id,old_paid_value,remark) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	    ...
	    ps.setString(17,cdrRecord.sub_id);
	    ps.setInt(18,cdrRecord.old_paid_value);
	    ps.setString(19,cdrRecord.remark);
		ps.execute();
		ps.close();
	}
